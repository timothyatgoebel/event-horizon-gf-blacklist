...
class EH_GFB_Sync {
    ...
    public function get_status() : array {
        ...
        $warnings = array();
        if ( empty( get_option( EH_GFB_Admin::OPT_CONTENT_URL, '' ) ) ) { $warnings[] = __( 'Content CSV URL is not set.', 'event-horizon-gf-blacklist' ); }
        if ( empty( get_option( EH_GFB_Admin::OPT_EMAIL_URL, '' ) ) ) { $warnings[] = __( 'Email CSV URL is not set.', 'event-horizon-gf-blacklist' ); }
        ...
        if ( ! isset( $status['content_count'] ) ) { $status['content_count'] = count( $this->get_cached_list( 'content' ) ); }
        if ( ! isset( $status['email_count'] ) ) { $status['email_count'] = count( $this->get_cached_list( 'email' ) ); }
    ...
    private function sync_one( string $type, string $url, bool $force, int $has_header ) : bool {
        $url = trim( $url );
        if ( $url === '' ) { return true; } // Not configured: not an error.
        ...
        if ( ! $force ) {
            if ( ! empty( $cache['etag'] ) ) { $headers['If-None-Match'] = (string) $cache['etag']; }
            if ( ! empty( $cache['last_modified'] ) ) { $headers['If-Modified-Since'] = (string) $cache['last_modified']; }
        }
    ...
    private function parse_csv_rules( string $csv, int $has_header ) : array {
        ...
        foreach ( $lines as $line ) {
            ...
            if ( $has_header && $row_index === 1 ) { continue; }
